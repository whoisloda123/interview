 ...
 *  3.work线程执行初始任务后，一直从队列里面取数据直到为null销毁线程，取的时候
 ...
 *  5.销毁销毁线程的时候，会尝试调用一下terminated（线程池结束的时候）扩展方法
 *  6.shutdown中断所有线程，runStatus为shutdown，不能新添加线程，会继续执行任务，shutdownnow中断所有线程，runStatus为shutdown，不能新添加线程和执行任务
