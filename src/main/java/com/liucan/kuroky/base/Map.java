 ...
 *    1.8 是用尾插法 head 和 tail 来保证链表的顺序和之前一样，这样就不会产生循环引用都会出现数据被覆盖的情况
 ...
 * 一.put 流程
 *  a.看数组是否被初始化，没有则初始化数组
 *  b.看通过 hash 获取到数组的位置是否有值，如果没有则 cas 插入
 *  c.如果节点是否在扩容，如果在扩容在加入到扩容里面去
 *  d.synchronized 锁住当前桶，插入到链表或红黑树尾部，如果链表长度大于 8 则会转换为红黑树
 *  e.看是否需要扩容，如果当前正在扩容，则加入到扩容里面，和其他线程一起扩容（会根据 cpu 数和桶大小来平均分给每个线程处理扩容桶数，默认最小是 16）
 * 二.get 不需要锁，因为 value 是 volatile 类型
 * 三.获取 size 的实现方式类似于 {@link LongAdder} 实现方式，比 CAS 效率要高一些
 * @author liucan
 * @date 5/24/21
 */
