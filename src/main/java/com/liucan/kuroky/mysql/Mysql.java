 ...
 *     e.深分页优化
 *          1.在SELECT * FROM table ORDER BY id LIMIT 1000000, 10;情况，会先查出先查询100000010条，然后丢弃掉前100000条，效率很慢
 *          2.可以通过前端只支持一页一页查，然后保存上一次查询的id，用于下一次查询
 *          3.可以先查询出最近的主键索引，然后查询大于主键索引的多少条数，select * from table where id >= (select id from table 1000000 1) limit 10
 *
